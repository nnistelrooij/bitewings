...
from bitewings.data.transforms import RandomBitewingFlip


@MODELS.register_module()
class LingyunTTAModel(DetTTAModel):
    ...
    def _merge_single_sample(
        ...
        img_metas = []
        convert_to_cpu = True
        label_map = torch.from_numpy(RandomBitewingFlip.label_map).to(data_samples[0].pred_instances.labels)
        for data_sample in data_samples:
            ...
            flipped_labels = label_map[labels]
        ...
        det_masks = merged_masks[keep_idxs]

        finding_bboxes = det_bboxes[det_labels >= 32]
        finding_labels = det_labels[det_labels >= 32]
        finding_masks = det_masks[det_labels >= 32]

        tooth_bboxes = det_bboxes[det_labels < 32]
        tooth_labels = det_labels[det_labels < 32]
        tooth_masks = det_masks[det_labels < 32]

        tooth_bboxes, keep_idxs = batched_nms(tooth_bboxes[:, :-1], tooth_bboxes[:, -1],
                                            tooth_labels, {**self.tta_cfg.nms, 'class_agnostic': True})
        tooth_labels = tooth_labels[keep_idxs]
        tooth_masks = tooth_masks[keep_idxs]

        results = InstanceData()
        _det_bboxes = torch.cat((finding_bboxes, tooth_bboxes)).clone()
        results.bboxes = _det_bboxes[:, :-1]
        results.scores = _det_bboxes[:, -1]
        results.labels = torch.cat((finding_labels, tooth_labels))
        results.masks = torch.cat((finding_masks, tooth_masks))
